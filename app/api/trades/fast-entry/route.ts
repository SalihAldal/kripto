...
import { recordAIAnalysisRejectSample, resolveAnalysisHorizon } from "@/src/server/ai/ai-analysis-memory.service";
...
const schema = z.object({
  ...
  scanLimit: z.number().int().min(100).max(500).optional(),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const paperLearningLane = env.EXECUTION_MODE === "paper";
    ...
    const best = await getBestFastEntry({
      scanLimit: parsed.data.scanLimit ?? 100,
      forcePaperProfile: paperLearningLane,
      scanCycles: paperLearningLane ? 4 : 3,
    ...
    if (!best.selected || !best.selected.ai) {
      const horizon = resolveAnalysisHorizon(duration);
      await Promise.all(
        (best.diagnostics?.sampleRejected ?? []).slice(0, 6).map((row) =>
          recordAIAnalysisRejectSample({
            ...
            reason: best.reason ?? row.notes.join(", ") ?? "NO_FAST_ENTRY_SELECTION",
            metadata: {
              source: "fast-entry-no-selection",
              maxDurationSec: duration,
              spreadPercent: row.spreadPercent,
              shortFlowImbalance: row.shortFlowImbalance,
              tradeVelocity: row.tradeVelocity,
              fakeSpikeScore: row.fakeSpikeScore,
              notes: row.notes,
    ...
    if (env.BINANCE_PLATFORM === "tr" && !selected.context.symbol.toUpperCase().endsWith("TRY")) {
      return apiOkFromRequest(request, {
        ...
        scannedAt: best.scannedAt,
        evaluated: best.evaluated,
        reason: tr ? "TRY disi parite engellendi. USDT/TRY whitelist ayari gerekli." : "Non-TRY pair blocked. Allow USDT/TRY whitelist to proceed.",
      });
    }
    ...
    const execution = execute
      ? await openTrade({
          ...
          learningLane: paperLearningLane,
        })
      : null;
    ...
    const executionReason =
      ...
      (executionFailed
        ...
        : undefined);
    if (executionFailed) {
      await recordAIAnalysisRejectSample({
        userId: access.user.id,
        symbol: selected.context.symbol,
        horizon: resolveAnalysisHorizon(duration),
        marketRegime: ai.decisionPayload?.marketMode,
        decision: ai.finalDecision,
        confidence: ai.finalConfidence,
        reason: executionReason ?? "EXECUTION_REJECTED",
        metadata: {
          source: "fast-entry-execution-reject",
          maxDurationSec: duration,
          scannerScore: selected.score.score,

...
async function loadBalancePayload(locale: "tr" | "en", forceRefresh = false): Promise<BalanceApiPayload> {
  ...
  balanceInFlight = (async () => {
    ...
    if (balanceCache) {
      ...
      const suppressNetworkError = payload.errorCode === "NETWORK";
      return {
        ...balanceCache.payload,
        error: suppressNetworkError ? null : (payload.error ?? balanceCache.payload.error),
        errorCode: suppressNetworkError ? null : (payload.errorCode ?? balanceCache.payload.errorCode),
        errorHint: suppressNetworkError ? null : (payload.errorHint ?? balanceCache.payload.errorHint),
        rawError: suppressNetworkError ? null : (payload.rawError ?? balanceCache.payload.rawError),
        updatedAt: new Date().toISOString(),
      };
    }
    return payload;

...
const runBacktestSchema = z.object({
  ...
  symbols: z.array(z.string().min(5)).min(1).max(250),
  ...
  scanUniverseSize: z.number().int().min(20).max(300).default(100),
  forceSimulatedFills: z.boolean().default(true),
  intervalMinutes: z.number().int().min(1).max(240).default(1),
  trailingStartPercent: z.number().min(0).max(50).optional(),
  trailingGapPercent: z.number().min(0).max(50).optional(),
});

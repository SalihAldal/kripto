...
import { AutoCycleReportPanel, type AutoCycleReportItem } from "@/src/features/dashboard/components/auto-cycle-report-panel";
...
function holdLabel(value: number) {
  return AUTO_HOLD_OPTIONS.find((option) => option.value === value)?.label ?? formatSecondsToMinSec(value);
}
...
export default function DashboardPage() {
  ...
  const [shortTermReport, setShortTermReport] = useState<string | null>(null);
  ...
  const [autoMaxHoldSec, setAutoMaxHoldSec] = useState(3600);
  const [autoCycleReports, setAutoCycleReports] = useState<AutoCycleReportItem[]>([]);
  ...
  const activeScannerRow = useMemo(
    () => scannerData.find((row) => row.symbol === activeSymbol) ?? scannerData[0] ?? null,
    [scannerData, activeSymbol],
  );
  ...
  const upsertAutoCycleReport = useCallback((cycleNo: number, patch: Partial<AutoCycleReportItem>) => {
    const now = new Date().toISOString();
    setAutoCycleReports((prev) => {
      const index = prev.findIndex((item) => item.cycleNo === cycleNo);
      if (index === -1) {
        return [
          {
            id: `auto-cycle-report-${cycleNo}-${Date.now()}`,
            cycleNo,
            status: patch.status ?? "running",
            maxDurationSec: patch.maxDurationSec ?? autoMaxHoldSec,
  ...
  const runAnalyze = async () => {
    ...
    setLoading(true);
    try {
      const scanResult = await apiPost<FastEntryResponse>("/api/trades/fast-entry", { execute: false, scanLimit: 100 });
      const bestSymbol = scanResult.selected?.symbol;
      const report = scanResult.selected?.decisionPayload?.shortTermReport ?? null;
      if (!scanResult.ok || !bestSymbol) {
        pushToast(scanResult.reason ?? t("dashboard.providerError"), "info");
        return;
      }
      if (report) setShortTermReport(report);
  ...
  const executeTrade = useCallback(async (options?: { forceSpot?: boolean; source?: "manual" | "auto" }) => {
    ...
    const effectiveMaxDurationSec = autoMaxHoldSec;
    ...
    if (source === "auto") {
      ...
      upsertAutoCycleReport(nextCycle, {
        status: "running",
        maxDurationSec: effectiveMaxDurationSec,
        reason: "Analiz ve scanner taramasi basladi.",
      });
    }
    ...
    try {
      ...
      const report = response.selected?.decisionPayload?.shortTermReport ?? null;
      if (report) setShortTermReport(report);
      if (source === "auto" && autoRunningCycleRef.current) {
        upsertAutoCycleReport(autoRunningCycleRef.current, {
          symbol: response.selected?.symbol ?? response.execution?.symbol ?? null,
          decision: response.selected?.decision ?? null,
          aiConfidence: response.selected?.aiConfidence ?? null,
          scannerScore: response.selected?.scannerScore ?? null,
          analysis: report,
          reason: response.reason ?? response.execution?.rejectReason ?? undefined,
          maxDurationSec: effectiveMaxDurationSec,
        });
      }
      if (!response.ok) {
        ...
        if (source === "auto") {
          ...
          if (failedCycle) {
            ...
            upsertAutoCycleReport(failedCycle, {
              status: "skipped",
              reason: response.reason ?? response.execution?.rejectReason ?? "Uygun islem bulunamadi / acilis reddedildi.",
              result: "skipped",
              analysis: report,
          ...
          setAutoNextRunAt(Date.now() + 3_000);
        }
        return;
      }
      if (response.execution?.opened) {
        ...
        if (source === "auto" && autoRunningCycleRef.current) {
          ...
          upsertAutoCycleReport(autoRunningCycleRef.current, {
            ...
            symbol,
            executionId,
            entryPrice: entryFromExecution > 0 ? entryFromExecution : lastTradePrice,
            takeProfitPrice: Number(details?.takeProfitPrice ?? 0) > 0 ? Number(details?.takeProfitPrice) : null,
            stopLossPrice: Number(details?.stopLossPrice ?? 0) > 0 ? Number(details?.stopLossPrice) : null,
            reason: "Pozisyon acildi, satis/kapanis monitoru izleniyor.",
            analysis: report,
          });
        }
        void Promise.all([loadOverview(), reloadScanner()]);
        ...
        if (source === "auto") {
          ...
          if (failedCycle) {
            ...
            upsertAutoCycleReport(failedCycle, {
              status: "skipped",
              reason: response.execution?.rejectReason ?? response.reason ?? "Islem acilamadi.",
              result: "skipped",
              analysis: report,
              closedAt: new Date().toISOString(),
            });
      ...
      if (source === "auto") {
        ...
        autoRunningCycleRef.current = null;
        setAutoNextRunAt(Date.now() + 5_000);
      }
    } finally {
      setLoading(false);
    }
  }, [autoMaxHoldSec, maxCoins, pushToast, reloadScanner, t, tradeAmountTry, tradeAmountUsdt, tradeLeverage, activeSymbol, overview?.lastTrade, loadOverview, pushAutoCycleNotification, upsertAutoCycleReport]);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      source = new EventSource(withBasePath("/api/trades/stream"));
      source.onmessage = (raw) => {
        try {
          ...
          if (sameFlow) {
            ...
            if (event.stage === "completed" && event.status === "SUCCESS") {
              ...
              if (autoSpotEnabled && autoRunningCycleRef.current) {
                ...
                upsertAutoCycleReport(autoRunningCycleRef.current, {
                  status: "opened",
                  symbol: event.symbol ? toTryDisplaySymbol(event.symbol) : undefined,
                  executionId: event.executionId ?? undefined,
                  entryPrice: Number(ctx?.buyPrice ?? 0) > 0 ? Number(ctx?.buyPrice) : undefined,
                  takeProfitPrice: Number(ctx?.takeProfitPrice ?? 0) > 0 ? Number(ctx?.takeProfitPrice) : undefined,
                  feeTotal: Number(ctx?.buyFee ?? 0) > 0 ? Number(ctx?.buyFee) : undefined,
                  reason: "Alim tamamlandi, pozisyon monitoru aktif.",
                });
              }
            }
          ...
          if (event.stage === "settlement" && event.status === "SUCCESS" && event.context?.tradeSummary) {
            ...
            const pnl = event.context.pnl as { feeTotal?: number } | undefined;
            const closedSummary = { ...s, feeTotal: Number(s.feeTotal ?? pnl?.feeTotal ?? 0) };
            ...
            setClosedTradeQueue((prev) => [...prev, closedSummary]);
            setFlowStatus("idle");
            setFlowExecutionId(null);
            setFlowSymbol(null);
            setFlowExpectedSellAt(null);
            ...
            if (autoSpotEnabled && autoRunningCycleRef.current) {
              ...
              pushAutoCycleNotification(
                `Cycle #${completedCycle} tamamlandi: ${s.symbol} kapandi, net PnL ${pnlLabel}, ROE ${Number(s.roePercent ?? 0).toFixed(4)}%.`,
                s.netPnl >= 0 ? "success" : "warning",
              );
              upsertAutoCycleReport(completedCycle, {
                status: "closed",
                symbol: s.symbol,
                ...
                feeTotal: Number(s.feeTotal ?? pnl?.feeTotal ?? 0),
                result: s.outcome ?? (s.netPnl >= 0 ? "profit" : "loss"),
                reason: `Kapanis nedeni: ${s.closeReason}`,
                closedAt: new Date().toISOString(),
              });
              autoRunningCycleRef.current = null;
            }
  ...
  }, [autoSpotEnabled, flowExecutionId, flowSymbol, liveMonitoringEnabled, loadOverview, localeTag, pushAutoCycleNotification, pushToast, t, upsertAutoCycleReport]);
            ...
            <div className="mt-3 overflow-hidden rounded-lg border border-outline-variant/30">
              <table className="w-full text-sm">
                <tbody>
                  ...
                  <tr className="border-b border-outline-variant/20"><td className="px-3 py-2 text-on-surface-variant">Fee</td><td className="px-3 py-2 text-right font-semibold">{Number(closedTradeModal.feeTotal ?? 0).toFixed(8)}</td></tr>
                  <tr className="border-b border-outline-variant/20"><td className="px-3 py-2 text-on-surface-variant">Kar/Zarar %</td><td className={`px-3 py-2 text-right font-bold ${Number(closedTradeModal.roePercent ?? 0) >= 0 ? "text-secondary" : "text-tertiary"}`}>{closedTradeModal.roePercent === undefined ? "-" : `${closedTradeModal.roePercent >= 0 ? "+" : ""}${closedTradeModal.roePercent.toFixed(4)}%`}</td></tr>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-12 gap-4">
        ...
        <div className="xl:col-span-4 space-y-4">
          <QuickTradeActionPanel
            ...
            maxHoldSec={autoMaxHoldSec}
            onMaxHoldSecChange={setAutoMaxHoldSec}
            ...
            shortTermReport={shortTermReport ?? undefined}
            pumpIntensity={activeScannerRow?.pumpIntensity}
            pumpRisk={activeScannerRow?.pumpRisk}
            volumeSpikePercent={activeScannerRow?.volumeSpikePercent}
            socialSentimentScore={activeScannerRow?.socialSentimentScore}
            newsSentiment={activeScannerRow?.newsSentiment}
            progressText={
              flowStatus === "position-open"
          ...
          />
          <AutoCycleReportPanel
            items={autoCycleReports}
            active={autoSpotEnabled}
            maxDurationLabel={holdLabel(autoMaxHoldSec)}
          />

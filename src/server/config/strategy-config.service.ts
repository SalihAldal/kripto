...
import { upsertRiskConfig } from "@/src/server/repositories/risk.repository";
...
const tradeSchema = z.object({
  ...
  trailingStartPercent: z.number().min(0).max(50),
  trailingGapPercent: z.number().min(0).max(50),
...
const aiSchema = z.object({
  ...
  targetUpdateConfidence: z.number().min(0).max(100),
});
...
const STRATEGY_PRESETS: Record<StrategyPresetName, {
  minConfidenceScore: number;
  trailingStartPercent: number;
  trailingGapPercent: number;
  maxOpenPositions: number;
  maxCoinAllocationPercent: number;
  dailyLossLimitPercent: number;
  targetUpdateThreshold: number;
}> = {
  Conservative: {
    minConfidenceScore: 82,
    trailingStartPercent: 1.4,
    trailingGapPercent: 0.7,
    maxOpenPositions: 1,
    maxCoinAllocationPercent: 10,
    dailyLossLimitPercent: 1.5,
    targetUpdateThreshold: 88,
  },
  Balanced: {
    minConfidenceScore: 75,
    trailingStartPercent: 1.1,
    trailingGapPercent: 0.55,
    maxOpenPositions: 2,
    maxCoinAllocationPercent: 15,
    dailyLossLimitPercent: 2.5,
    targetUpdateThreshold: 82,
  },
  Aggressive: {
    minConfidenceScore: 68,
    trailingStartPercent: 0.9,
    trailingGapPercent: 0.45,
    maxOpenPositions: 3,
    maxCoinAllocationPercent: 25,
    dailyLossLimitPercent: 4,
    targetUpdateThreshold: 78,
  },
  Scalping: {
    ...
    maxOpenPositions: 4,
    maxCoinAllocationPercent: 12,
    dailyLossLimitPercent: 2,
    targetUpdateThreshold: 80,
  },
  Momentum: {
    minConfidenceScore: 66,
    trailingStartPercent: 1.6,
    trailingGapPercent: 0.75,
    maxOpenPositions: 2,
    maxCoinAllocationPercent: 18,
    dailyLossLimitPercent: 2.8,
    targetUpdateThreshold: 84,
  },
};
...
const DEFAULT_CONFIG: StrategyConfig = {
  trade: {
    ...
    stopLossPercent: 1,
    trailingStartPercent: 1.2,
    trailingGapPercent: 0.6,
...
export async function getActiveStrategyConfig(options?: { bypassCache?: boolean }) {
  ...
  throw new Error(initial.errors.join(", "));
}
...
export function listStrategyPresets() {
  return Object.entries(STRATEGY_PRESETS).map(([name, preset]) => ({
    name,
    ...preset,
  }));
}

export async function applyStrategyPreset(input: { userId: string; preset: StrategyPresetName; note?: string }) {
  const preset = STRATEGY_PRESETS[input.preset];
  ...
  const config: StrategyConfig = {
    ...
    trade: {
      ...
      trailingStartPercent: preset.trailingStartPercent,
      trailingGapPercent: preset.trailingGapPercent,
    },
    ai: {
      ...base.ai,
      aiScoreThreshold: preset.minConfidenceScore,
      targetUpdateConfidence: preset.targetUpdateThreshold,
    },
  };
  const saved = await saveStrategyConfig({
    config,
    updatedBy: input.userId,
    note: input.note ?? `preset:${input.preset}`,
  });
  if (!saved.ok) return saved;

  await upsertRiskConfig(input.userId, {
    maxOpenPositions: preset.maxOpenPositions,
    maxDailyLossPercent: preset.dailyLossLimitPercent,
    metadata: {
      maxBalancePercentPerCoin: preset.maxCoinAllocationPercent,
    },
  });

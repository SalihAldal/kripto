...
export async function getRuntimeStrategyParams(): Promise<RuntimeStrategyParams> {
  try {
    ...
    return {
      ...
      trailingStartPercent: clamp(
        Number(cfg.trade.trailingStartPercent ?? FALLBACK_RUNTIME_PARAMS.trailingStartPercent),
        0,
        50,
      ),
      trailingGapPercent: clamp(
        Number(cfg.trade.trailingGapPercent ?? FALLBACK_RUNTIME_PARAMS.trailingGapPercent),
        0,
        50,
      ),
      targetUpdateConfidence: clamp(
        Number(cfg.ai.targetUpdateConfidence ?? FALLBACK_RUNTIME_PARAMS.targetUpdateConfidence),
        0,
        100,
      ),

...
function createContextSpecialistOutput(input: AIAnalysisInput, provider: string): AIModelOutput {
  ...
  const highImpactNews = Boolean(input.marketSignals?.macroHighImpactNews ?? false);
  const macroUncertainty = Number(input.marketSignals?.macroUncertaintyLevel ?? 0);
  const isBtcSymbol = input.symbol.startsWith("BTC") || input.symbol.endsWith("BTC");
  ...
  const marketAlignment = clampScore(
    50 + shortFlow * 30 + orderbookSkew * 20 - Math.max(0, btcDominanceBias) * 18 - (macroUncertainty > 60 ? 8 : 0),
  );
  ...
  if (highImpactNews) redFlags.push("High impact haber penceresi");
  if (macroUncertainty >= 70) redFlags.push("Makro belirsizlik yuksek");
  if (btcDominanceBias >= 0.35 && !isBtcSymbol) redFlags.push("BTC dominance artisi: altcoin riskli");

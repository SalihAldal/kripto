...
function resolveTrendDirection(input: AIAnalysisInput, indicators: Readonly<IndicatorSnapshot>) {
  ...
  const emaBull = indicators.ema20 > indicators.ema50 && indicators.ema50 > indicators.ema200;
  const emaBear = indicators.ema20 < indicators.ema50 && indicators.ema50 < indicators.ema200;
  if (emaBull) return "BULLISH";
  if (emaBear) return "BEARISH";
...
function buildTechnicalSpecialistOutput(input: AIAnalysisInput, label: string): AIModelOutput {
  ...
  if (trendDirection === "BULLISH" && input.lastPrice < ind.ema200) {
    redFlags.push("EMA200 altinda long riskli");
  }
  if (trendDirection === "BEARISH" && input.lastPrice > ind.ema200) {
    redFlags.push("EMA200 ustunde short riskli");
  }

  const emaBull = ind.ema20 > ind.ema50 && ind.ema50 > ind.ema200;
  const emaBear = ind.ema20 < ind.ema50 && ind.ema50 < ind.ema200;
  const structureBull = emaBull && ind.macd > ind.signalLine;
  const structureBear = emaBear && ind.macd < ind.signalLine;
  const pullbackEntry =
  ...
  const confidenceBase =
    (structureBull || structureBear ? 26 : 8) +
  ...
  const bullishFactors = [
    ...confirmations,
    structureBull ? "Bullish EMA20/50/200 alignment" : "",
    ind.breakoutUp ? "Breakout continuation signal" : "",
  ].filter(Boolean);
  const bearishFactors = [
    structureBear ? "Bearish EMA20/50/200 alignment" : "",
  ...
  return {
    ...
    metadata: {
      ...
      indicators: {
        ...
        ema20: Number(ind.ema20.toFixed(8)),
        ema50: Number(ind.ema50.toFixed(8)),
        ema200: Number(ind.ema200.toFixed(8)),

...
function toFinitePercent(value: unknown, fallback: number) {
  const parsed = typeof value === "number" ? value : Number(value);
...
function resolveRegimePolicy(regime: AIAnalysisInput["marketRegime"] | undefined) {
  ...
  if (mode === "STRONG_BULLISH_TREND" || mode === "ROCKET_PUMP") {
    return {
      mode,
      minCompositeDelta: -5,
      minTechDelta: -3,
...
function toTimeframePayload(input: AIAnalysisInput["multiTimeframe"]) {
  ...
  if (!hasLayered) {
    ...
    const legacyM1 = legacy.entry.m1 ?? legacy.entry.m5;
    const lowerQuality =
  ...
  return {
    ...
    lower: {
      m1: {
        direction: input.lower.m1.direction,
        strength: input.lower.m1.strength,
        slopePercent: input.lower.m1.slopePercent,
      },
      m15: {
    ...
    entry: {
      m1: {
        direction: input.entry.m1.direction,
        strength: input.entry.m1.strength,
        slopePercent: input.entry.m1.slopePercent,
      },
      m15: {
...
function scoreTechnical(input: AIAnalysisInput, provider: AIProviderResult | null): AIRoleScore {
  ...
  const emaBear = ind.ema20 < ind.ema50 && ind.ema50 < ind.ema200;
  if (emaBull) {
    score += 9;
    rationale.push("EMA20/50/200 hizali (bullish)");
  } else if (emaBear) {
    score -= 9;
    rationale.push("EMA20/50/200 hizali (bearish)");
  } else {
    score -= 4;
    rationale.push("EMA hizasi net degil");
  }
  if (input.lastPrice >= ind.ema200) {
    score += 4;
    rationale.push("Fiyat EMA200 uzeri");
  } else {
    score -= 4;
    rationale.push("Fiyat EMA200 alti");
  }
...
function scoreRisk(
  ...
  if (rr !== null && rr < env.AI_HYBRID_MIN_RR_RATIO) {
    score -= 14;
...
export function buildHybridDecision(input: {
  ...
  const strategyExecutionMode = String(
    strategyParams.executionMode ?? strategyParams.mode ?? strategyParams.tradeMode ?? "",
  ).toLowerCase();
  const paperMode =
    strategyParams.paperMode === true || strategyExecutionMode === "paper" || env.EXECUTION_MODE === "paper";
  const paperRelaxed = paperMode;
  const runtimeMinTechScore = Math.max(
    0,
    Number(strategyParams.technicalMinScore ?? env.AI_HYBRID_MIN_TECH_SCORE) - (paperRelaxed ? 4 : 0),
  );
  const runtimeMinSentimentScore = Math.max(
    0,
    Number(strategyParams.sentimentMinScore ?? env.AI_HYBRID_MIN_SENTIMENT_SCORE) - (paperRelaxed ? 4 : 0),
  );
  const runtimeMinCompositeScore = Math.max(
    0,
    Number(strategyParams.consensusMinScore ?? env.AI_HYBRID_MIN_COMPOSITE_SCORE) - (paperRelaxed ? 12 : 0),
  );
  const runtimeMinQualityScore = Math.max(
    0,
    Number(strategyParams.consensusMinScore ?? env.EXECUTION_MIN_TRADE_QUALITY_SCORE ?? 60) - (paperRelaxed ? 4 : 0),
  );
  ...
  const baseMinProfitPercent = toFinitePercent(
    strategyParams.minProfitPercent ?? env.EXECUTION_TARGET_MIN_PROFIT_PERCENT ?? 1,
    1,
  );
  const protectedMinProfitPercent = toFinitePercent(resolveMinimumProtectedProfitPercent(), 0);
  const fallbackMinProfitPercent = Math.max(0.2, baseMinProfitPercent, protectedMinProfitPercent);
  const safeDefaultStopPercent = toFinitePercent(env.EXECUTION_DEFAULT_STOP_LOSS_PERCENT, fallbackMinProfitPercent);
  const fallbackStopPercent = Math.max(0.2, fallbackMinProfitPercent * 0.75, safeDefaultStopPercent * 0.75);
  const directionalBias = decisionScore(technical.decision) + decisionScore(sentiment.decision);
  const dominantTrend = String(input.analysisInput.multiTimeframe?.dominantTrend ?? "RANGE");
  const targetValue = Number(target ?? NaN);
  const stopValue = Number(stop ?? NaN);
  const resolvedBiasSide = (() => {
    if (Number.isFinite(targetValue) && Number.isFinite(lastPrice)) {
      if (targetValue > lastPrice) return "BUY";
      if (targetValue < lastPrice) return "SELL";
    }
    if (Number.isFinite(stopValue) && Number.isFinite(lastPrice)) {
      if (stopValue < lastPrice) return "BUY";
      if (stopValue > lastPrice) return "SELL";
    }
    if (directionalBias > 0) return "BUY";
    if (directionalBias < 0) return "SELL";
    if (dominantTrend === "BEARISH") return "SELL";
    if (dominantTrend === "BULLISH") return "BUY";
    return "BUY";
  })();
  if (Number.isFinite(lastPrice) && lastPrice > 0) {
    if (resolvedBiasSide === "BUY") {
      if (!Number.isFinite(targetValue) || targetValue <= lastPrice) {
        target = Number((lastPrice * (1 + fallbackMinProfitPercent / 100)).toFixed(8));
      }
      if (!Number.isFinite(stopValue) || stopValue >= lastPrice) {
        stop = Number((lastPrice * (1 - fallbackStopPercent / 100)).toFixed(8));
      }
    } else {
      if (!Number.isFinite(targetValue) || targetValue >= lastPrice) {
        target = Number((lastPrice * (1 - fallbackMinProfitPercent / 100)).toFixed(8));
  ...
  if (Number.isFinite(lastPrice) && lastPrice > 0) {
    if (resolvedBiasSide === "BUY") {
      if (!Number.isFinite(target ?? NaN) || Number(target) <= lastPrice) {
        target = Number((lastPrice * (1 + fallbackMinProfitPercent / 100)).toFixed(8));
      }
      if (!Number.isFinite(stop ?? NaN) || Number(stop) >= lastPrice) {
        stop = Number((lastPrice * (1 - fallbackStopPercent / 100)).toFixed(8));
      }
      if (Number.isFinite(target ?? NaN) && Number.isFinite(stop ?? NaN) && Number(target) <= Number(stop)) {
        stop = Number((lastPrice * (1 - fallbackStopPercent / 100)).toFixed(8));
      }
    } else {
      if (!Number.isFinite(target ?? NaN) || Number(target) >= lastPrice) {
  ...
  const regimeChaosProbability = Number(input.analysisInput.marketSignals?.regimeChaosProbability ?? 0);
  const regimeLifecyclePhase = String(input.analysisInput.marketSignals?.regimeLifecyclePhase ?? "UNKNOWN");
  ...
  const uncertainDirection = paperRelaxed
    ? (regimePolicy.mode === "LOW_VOLUME_DEAD_MARKET" ||
        regimePolicy.mode === "NEWS_DRIVEN_UNSTABLE" ||
        (regimePolicy.mode === "HIGH_VOLATILITY_CHAOS" && regimeTransitionProbability >= 80) ||
        regimeFlipRisk >= 85 ||
        (regimeChopWarning && regimeTransitionProbability >= 75))
    : (regimePolicy.mode === "LOW_VOLUME_DEAD_MARKET" ||
        regimePolicy.mode === "NEWS_DRIVEN_UNSTABLE" ||
        regimePolicy.mode === "HIGH_VOLATILITY_CHAOS" ||
        regimeTransitionProbability >= 68 ||
        regimeFlipRisk >= 72 ||
        regimeChopWarning ||
        ((regimePolicy.mode === "RANGE_SIDEWAYS" || regimePolicy.mode === "LOW_VOLATILITY_CALM") &&
  ...
  const recoverableEntryWindow =
    (mtfTrendAligned &&
      !mtfConflict &&
      mtfAlignmentScore >= (paperRelaxed ? 50 : 72) &&
      (regimePolicy.mode === "RANGE_SIDEWAYS" ||
        regimePolicy.mode === "LOW_VOLATILITY_CALM" ||
        regimePolicy.mode === "WEAK_BULLISH_TREND" ||
  ...
  const sentimentFlags = Array.isArray(sentimentMeta.redFlags)
    ...
    : [];
  const macroHighImpactNews = Boolean(input.analysisInput.marketSignals?.macroHighImpactNews ?? false);
  const macroUncertainty = Number(input.analysisInput.marketSignals?.macroUncertaintyLevel ?? 0);
  const macroRisk = macroUncertainty >= 70;
  const btcDominanceBias = Number(input.analysisInput.marketSignals?.btcDominanceBias ?? 0);
  const isBtcSymbol = input.analysisInput.symbol.startsWith("BTC") || input.analysisInput.symbol.endsWith("BTC");
  const btcDominanceHeadwind = btcDominanceBias >= 0.35 && !isBtcSymbol;
  const newsComplex =
    news === "NEGATIVE" || sentimentFlags.length >= 2 || sentimentMeta.tradeSupportive === false || macroHighImpactNews;
  const aggressiveSentimentBlock = newsComplex && regimePolicy.strategy === "aggressive";
  ...
  const change1m = Number(input.analysisInput.marketSignals?.change1m ?? 0);
  const change5m = Number(input.analysisInput.marketSignals?.change5m ?? 0);
  const change15m = Number(input.analysisInput.marketSignals?.change15m ?? 0);
  const volumeSpikeRatio = Number(input.analysisInput.marketSignals?.volumeSpikeRatio ?? 0);
  const futuresRiskScore = Number(input.analysisInput.marketSignals?.futuresRiskScore ?? 0);
  const leverageStressScore = Number(input.analysisInput.marketSignals?.leverageStressScore ?? 0);
  const leveragedTrapProbability = Number(input.analysisInput.marketSignals?.leveragedTrapProbability ?? 0);
  const squeezeProbability = Number(input.analysisInput.marketSignals?.squeezeProbability ?? 0);
  const manipulationPressureScore = Number(input.analysisInput.marketSignals?.manipulationPressureScore ?? 0);
  const futuresIntent = String(input.analysisInput.marketSignals?.futuresIntent ?? "NEUTRAL");
  const futuresIntelDegraded = Boolean(input.analysisInput.marketSignals?.futuresIntelDegraded ?? false);
  const pumpCandidate = Boolean(input.analysisInput.marketSignals?.pumpCandidate ?? false);
  const pumpStage = String(input.analysisInput.marketSignals?.pumpStage ?? "");
  const pumpPriorityScore = Number(input.analysisInput.marketSignals?.pumpPriorityScore ?? 0);
  const change24h = Number(input.analysisInput.marketSignals?.change24h ?? 0);
  const wickDominance = Number(indicators.liquidity.wickDominance ?? 0);
  const wickSpikeDetected = wickDominance >= 0.62 || indicators.liquidity.breakoutTrap;
  const lowVolumeBreakout = Boolean(indicators.liquidity.lowVolumeBreakout);
  const stopHuntNeedsConfirm =
    Boolean(indicators.liquidity.stopHuntDetected) &&
    String(indicators.liquidity.sweepConfirmedDirection ?? "NONE") === "NONE";
  const singleCandleManipulation =
    wickDominance >= 0.68 &&
    Number(indicators.liquidity.previousWickDominance ?? 0) <= 0.32 &&
    (Math.abs(change1m) >= 0.9 || Math.abs(change5m) >= 2);
  const fastMoveDetected =
    Math.abs(change1m) >= 1.2 ||
    Math.abs(change5m) >= 2.6 ||
    Math.abs(change15m) >= 4.8;
  const fakeBreakoutHigh =
    indicators.liquidity.fakeBreakoutDetected ||
    Number(indicators.liquidity.fakeBreakoutRiskScore ?? 0) >= (paperRelaxed ? 88 : 62);
  const unhealthyVolumeSpike =
    !paperRelaxed &&
    volumeSpikeRatio >= 2.5 && (wickSpikeDetected || fakeBreakoutHigh || lateEntryDetected);
  const newsSpike =
    news !== "NEUTRAL" &&
    fastMoveDetected &&
    (volumeSpikeRatio >= 2 || Math.abs(change1m) >= 1.6 || Math.abs(change5m) >= 3.2);
  const manipulationRisk =
    fastMoveDetected && (unhealthyVolumeSpike || wickSpikeDetected || fakeBreakoutHigh || singleCandleManipulation);
  const futuresTrapRiskRaw =
    leveragedTrapProbability >= 72 ||
    futuresIntent === "LATE_LONG_TRAP" ||
    futuresIntent === "LATE_SHORT_TRAP" ||
    futuresIntent === "MANIPULATION_SQUEEZE_RISK";
  const futuresTrapRisk = futuresIntelDegraded ? false : futuresTrapRiskRaw;
  const futuresSqueezeRisk = futuresIntelDegraded ? false : squeezeProbability >= 76 || leverageStressScore >= 78;
  const unstableRegimeRisk =
    regimeChopWarning ||
    regimeUnstableBreakout ||
    regimeTransitionProbability >= 72 ||
    regimeFlipRisk >= 78 ||
    regimeChaosProbability >= 82;
  ...
  const canRelaxQualityThreshold =
    ...
    regimeTransitionProbability < 55 &&
    !regimeChopWarning;
  const isPumpStage = pumpStage === "EARLY" || pumpStage === "ACTIVE";
  const pumpMomentumStrong =
    change24h >= 6 &&
    Number(input.analysisInput.marketSignals?.shortMomentumPercent ?? 0) >= 0.12 &&
    Number(input.analysisInput.marketSignals?.shortFlowImbalance ?? 0) >= 0.06;
  const pumpQualityCandidate =
    pumpCandidate &&
    (isPumpStage || pumpPriorityScore >= 65) &&
    (pumpMomentumStrong || change24h >= 8);
  const adaptiveQualityFloor = pumpQualityCandidate
    ? Math.min(46, baseMinQualityThreshold)
    : Math.min(48, baseMinQualityThreshold);
  ...
  const minQualityFloor = pumpQualityCandidate
    ? Math.min(40, adaptiveQualityThreshold)
    : Math.min(44, baseMinQualityThreshold);
  const minQualityThreshold = clamp(
    adaptiveQualityThreshold - (pumpQualityCandidate ? 4 : 0),
    minQualityFloor,
    adaptiveQualityThreshold,
  );
  const minQualityStrongFloor = pumpQualityCandidate
    ? Math.min(env.AI_HYBRID_MIN_QUALITY_STRONG, 46)
    : env.AI_HYBRID_MIN_QUALITY_STRONG;
  const qualityStrong = paperRelaxed
    ? tradeQualityScore >= Math.max(minQualityThreshold, 42)
    : canRelaxQualityThreshold || pumpQualityCandidate
      ? tradeQualityScore >=
          Math.max(
            minQualityThreshold + 1,
            pumpQualityCandidate ? Math.min(52, minQualityStrongFloor) : Math.min(58, minQualityStrongFloor),
          )
      : tradeQualityScore >= Math.max(minQualityThreshold + 4, minQualityStrongFloor);
  ...
  const manipulationRiskScore = Number(indicators.liquidity.fakeBreakoutRiskScore ?? 0);
  const volumeSpike = Number(input.analysisInput.marketSignals?.volumeSpikeRatio ?? indicators.volumeBoost ?? 1);
  const manipulationHigh =
    manipulationRiskScore >= 70 || (wickDominance >= 0.66 && volumeSpike >= 1.8);
  const pumpQualityRelaxed = pumpQualityCandidate && !manipulationHigh && !fakeBreakoutHigh;
  const allowPumpOverride =
    (pumpQualityRelaxed &&
      tradeQualityScore >= Math.max(minQualityThreshold, paperRelaxed ? 42 : 46) &&
      (pumpPriorityScore >= (paperRelaxed ? 62 : 70) || change24h >= (paperRelaxed ? 7 : 10))) ||
    (paperRelaxed &&
      pumpQualityRelaxed &&
      tradeQualityScore >= Math.max(minQualityThreshold - 4, 38) &&
      (pumpPriorityScore >= 58 || change24h >= 6));
  const allowPumpRegimeOverride = allowPumpOverride && mtfAlignmentScore >= (paperRelaxed ? 48 : 52) && !mtfConflict;
  const paperTechFloor = Math.max(42, runtimeMinTechScore - 8);
  const paperSentimentFloor = Math.max(40, runtimeMinSentimentScore - 8);
  ...
  const regimeSuitable =
    ...
    ((!uncertainDirection || recoverableEntryWindow || allowPumpRegimeOverride) &&
      (!mtfRejectReason || allowPumpRegimeOverride));
  const minShortTermRr = paperRelaxed ? 0.5 : Math.max(env.AI_HYBRID_MIN_RR_RATIO, 1.1);
  const rrBad = rr !== null && rr < minShortTermRr && !allowPumpOverride;
  const rrValue = rr === null ? 0 : Number(rr.toFixed(4));
  const liquiditySweepRequired = Boolean(indicators.liquidity.liquiditySweepDetected);
  const expansionRequired = Boolean(indicators.expansionCandle);
  const pumpLiquidityOverride = pumpQualityCandidate && tradeQualityScore >= (paperRelaxed ? 50 : 60);
  const requireLiquiditySweep = !allowPumpOverride && !paperRelaxed && tradeQualityScore < (pumpQualityCandidate ? 54 : 62);
  const requireExpansion = !allowPumpOverride && !paperRelaxed && tradeQualityScore < (pumpQualityCandidate ? 58 : 65);
  const hardReject =
    riskVeto ||
    regimePolicy.forceSkip ||
    (volatilityExtreme && !paperRelaxed && regimePolicy.mode === "HIGH_VOLATILITY_CHAOS") ||
    (volumeWeak && regimePolicy.mode === "LOW_VOLUME_DEAD_MARKET") ||
    (fakeBreakoutHigh && !paperRelaxed) ||
    (lowVolumeBreakout && !paperRelaxed) ||
    (stopHuntNeedsConfirm && !paperRelaxed) ||
    (singleCandleManipulation && !paperRelaxed) ||
    (newsSpike && !paperRelaxed) ||
    (manipulationHigh && !paperRelaxed) ||
    (futuresRiskScore >= (paperRelaxed ? 92 : 86) && tradeQualityScore < (paperRelaxed ? 45 : 78) && !allowPumpOverride) ||
    (futuresTrapRisk && tradeQualityScore < (paperRelaxed ? 42 : 82) && !allowPumpOverride) ||
    (futuresSqueezeRisk && !liquiditySweepRequired && tradeQualityScore < (paperRelaxed ? 42 : 80) && !allowPumpOverride) ||
    (unstableRegimeRisk && tradeQualityScore < (paperRelaxed ? 42 : 84) && !allowPumpOverride) ||
    (requireLiquiditySweep && !liquiditySweepRequired && !pumpLiquidityOverride) ||
    (requireExpansion && !expansionRequired && !pumpLiquidityOverride) ||
    (macroHighImpactNews && !paperRelaxed && tradeQualityScore < 72) ||
    (macroRisk && !paperRelaxed && tradeQualityScore < 74) ||
    (btcDominanceHeadwind && !paperRelaxed && tradeQualityScore < 76) ||
    (lateEntryDetected && !allowPumpOverride && !paperRelaxed) ||
    rrBad ||
    tradeQualityScore < (allowPumpOverride ? (paperRelaxed ? 30 : 40) : (paperRelaxed ? 35 : 45)) ||
    (manipulationRisk && !paperRelaxed) ||
    unhealthyVolumeSpike;

  let finalDecision: AIDecision = "NO_TRADE";
  let consensusDecision: "BUY" | "WATCHLIST" | "NO-TRADE" | "REJECT" = "NO-TRADE";
  const minDirScore = paperRelaxed ? -0.1 : 0.2;
  if (!hardReject && regimeSuitable && technicalStrong && momentumSupportive && compositeOk && qualityStrong && dirScore >= minDirScore) {
    consensusDecision = "BUY";
    finalDecision = "BUY";
  } else if (!hardReject && regimeSuitable && technicalStrong && (!momentumSupportive || !qualityStrong || dirScore < minDirScore)) {
  ...
  const shortTermScore = computeShortTermScore(input.analysisInput, {
    finalDecision,
    finalConsensusDecision: consensusDecision,
    finalConsensusConfidence: 0,
    finalConfidence: 0,
    finalRiskScore: 0,
    score: 0,
    explanation: "",
  ...
  const baseEdgeScore = env.AI_HYBRID_MIN_EDGE_SCORE - (paperRelaxed ? 6 : 0);
  const minEdgeScore = clamp(baseEdgeScore - (pumpQualityCandidate ? 8 : 0), 45, 90);
  const strictScoreOk = shortTermScore.total >= minEdgeScore;
  const eliteConsensus =
    roleScores.every((row) => row.decision === "BUY") &&
    technical.score >= 68 &&
    sentiment.score >= 68 &&
    risk.score >= 68 &&
    qualityStrong &&
  ...
  const expectedMovePercent = (() => {
    const currentPrice = input.analysisInput.lastPrice;
    if (currentPrice > 0 && typeof target === "number" && target > 0) {
      if (finalDecision === "BUY") {
        return Math.max(0, ((target - currentPrice) / currentPrice) * 100);
      }
    }
    const momentum = Math.abs(Number(input.analysisInput.marketSignals?.shortMomentumPercent ?? 0));
    const volatility = Math.max(0, Number(input.analysisInput.volatility ?? 0));
    const regimeBoost = Number(input.analysisInput.marketRegime?.tpMultiplier ?? 1);
    const base = Math.max(momentum * 1.3, volatility * 0.9, 0.6);
    return base * regimeBoost;
  })();
  const durationSamples = input.allOutputs
    .map((row) => Number(row.output?.estimatedDurationSec ?? 0))
    .filter((value) => Number.isFinite(value) && value > 0);
  const estimatedDurationMinutes =
    durationSamples.length > 0
      ? Math.round(durationSamples.reduce((acc, value) => acc + value, 0) / durationSamples.length / 60)
      : 0;
  const minProfitPercent = Math.max(
    Number(strategyParams.minProfitPercent ?? env.EXECUTION_TARGET_MIN_PROFIT_PERCENT ?? 1),
    resolveMinimumProtectedProfitPercent(),
  );
  const maxProfitWindowMinutes = Number(strategyParams.maxProfitWindowMinutes ?? 120);
  const profitPercentOk =
    expectedMovePercent >= minProfitPercent ||
    (allowPumpOverride && expectedMovePercent >= minProfitPercent * 0.6) ||
    (paperRelaxed && expectedMovePercent >= minProfitPercent * 0.5);
  const profitWindowOk = estimatedDurationMinutes <= 0 || estimatedDurationMinutes <= maxProfitWindowMinutes;
  if (finalDecision === "BUY" && !strictScoreOk && !eliteConsensus && !allowPumpOverride && !paperRelaxed) {
    finalDecision = "NO_TRADE";
    consensusDecision = "NO-TRADE";
  }
  if (finalDecision === "BUY" && (!profitPercentOk || !profitWindowOk)) {
    finalDecision = "NO_TRADE";
    consensusDecision = "NO-TRADE";
  }
  ...
  if (finalDecision === "BUY" && aggressiveSentimentBlock) {
    consensusDecision = "WATCHLIST";
    finalDecision = "HOLD";
  }
  if (finalDecision === "BUY" && wickSpikeDetected) {
    consensusDecision = "WATCHLIST";
    finalDecision = "HOLD";
  }
  ...
  if (finalDecision === "BUY" && rr !== null && rr < minShortTermRr && !allowPumpOverride) {
    finalDecision = "NO_TRADE";
    consensusDecision = "REJECT";
  }

  const directionalVotes = roleScores.reduce((acc, row) => {
    if (row.decision === "BUY") acc.buy += 1;
    if (row.decision === "SELL") acc.sell += 1;
    return acc;
  }, { buy: 0, sell: 0 });
  const buyVotesRequired = allowPumpOverride || paperRelaxed ? 1 : 2;
  if (finalDecision === "BUY" && directionalVotes.buy < buyVotesRequired) {
    finalDecision = "NO_TRADE";
    consensusDecision = "NO-TRADE";
  }
  const liquidityRejectReason =
    indicators.liquidity.fakeBreakoutDetected || indicators.liquidity.fakeBreakoutRiskScore >= 62
      ...
      : (indicators.liquidity.nearUpperLiquidity || indicators.liquidity.nearLowerLiquidity) && !indicators.liquidity.stopHuntDetected
        ...
        : undefined;
  if (finalDecision === "BUY" && liquidityRejectReason && !eliteConsensus && !paperRelaxed) {
    finalDecision = "NO_TRADE";
    consensusDecision = "REJECT";
  }
  let noTradeReasonList = unique([
    uncertainDirection && !allowPumpRegimeOverride ? "Piyasa yonu belirsiz / market uygun degil" : "",
    mtfRejectReason && !allowPumpRegimeOverride ? "Timeframe'ler cakisiyor" : "",
    technicalWeak && !allowPumpOverride ? "Teknik setup zayif" : "",
    momentumWeak && !allowPumpOverride ? "Momentum guven vermiyor" : "",
    newsComplex ? "Haber/sentiment karmasik" : "",
    macroHighImpactNews ? "High impact haber: risk azalt" : "",
    macroRisk ? "Makro belirsizlik: agresif long azalt" : "",
    btcDominanceHeadwind ? "BTC dominance artisi: altcoin riskli" : "",
    newsSpike ? "News spike: teyit yok" : "",
    aggressiveSentimentBlock ? "Sentiment negatif: agresif trade kapali" : "",
    techStrongButOthersWeak ? "Teknik guclu ama diger AI destegi zayif" : "",
    fastMoveDetected ? "Son dakika fiyat degisimi cok hizli" : "",
    unhealthyVolumeSpike ? "Hacim patladi ama fiyat yapisi sagliksiz" : "",
    wickSpikeDetected ? "Wick spike tespit edildi (WAIT)" : "",
    lowVolumeBreakout ? "Low volume breakout: teyit yok" : "",
    singleCandleManipulation ? "Tek mum manipulasyon riski" : "",
    stopHuntNeedsConfirm ? "Stop hunt sonrasi teyit bekle" : "",
    volatilityExtreme ? "Volatilite asiri" : "",
    volumeWeak ? "Hacim yetersiz" : "",
    fakeBreakoutHigh ? "Fake breakout riski yuksek" : "",
    rrBad && !allowPumpOverride ? "Risk/odul orani kotu" : "",
    lateEntryDetected && !allowPumpOverride ? "Giris cok gec kalmis" : "",
    (requireLiquiditySweep && !liquiditySweepRequired) ? "Likidite sweep yok" : "",
    (requireExpansion && !expansionRequired) ? "Expansion fazi teyitsiz" : "",
    manipulationHigh ? "Manipulasyon riski yuksek" : "",
    futuresTrapRisk && !allowPumpOverride ? `Futures trap riski: ${futuresIntent}` : "",
    futuresSqueezeRisk && !allowPumpOverride ? "Futures squeeze/leverage stress yuksek" : "",
    futuresRiskScore >= 72 && !allowPumpOverride ? `Futures risk skoru yuksek (${futuresRiskScore.toFixed(1)})` : "",
    regimeChopWarning && !allowPumpRegimeOverride ? `Regime chop/rapid switching (${regimeLifecyclePhase})` : "",
    regimeTransitionProbability >= 62 && !allowPumpRegimeOverride ? `Regime transition probability yuksek (${regimeTransitionProbability.toFixed(1)})` : "",
    regimeFlipRisk >= 68 && !allowPumpRegimeOverride ? `Regime flip riski yuksek (${regimeFlipRisk.toFixed(1)})` : "",
    regimeUnstableBreakout && !allowPumpRegimeOverride ? "Unstable breakout condition" : "",
    !strictScoreOk && !eliteConsensus && !allowPumpOverride && !paperRelaxed ? `Edge skoru ${minEdgeScore} alti` : "",
    !profitPercentOk ? `Min net kar hedefi icin beklenen hareket %${minProfitPercent.toFixed(2)} alti` : "",
    !profitWindowOk ? "2 saat icinde hedefe ulasma zorlugu" : "",
    (finalDecision === "NO_TRADE" && (directionalVotes.buy > 0 || directionalVotes.sell > 0)) ? "En az 2 AI yonu ayni degil" : "",
  ]).filter(Boolean);
  const blockedByAi = unique([
    technical.decision === "NO_TRADE" ||
    (technicalWeak && !allowPumpOverride) ||
    (Boolean(mtfRejectReason) && !allowPumpRegimeOverride)
      ? "AI-1_TECHNICAL"
      : null,
    sentiment.decision === "NO_TRADE" || (momentumWeak && !allowPumpOverride) || newsComplex ? "AI-2_SENTIMENT" : null,
    risk.decision === "NO_TRADE" ||
    riskVeto ||
    volatilityExtreme ||
    volumeWeak ||
    (rrBad && !allowPumpOverride) ||
    fakeBreakoutHigh ||
    (futuresTrapRisk && !allowPumpOverride) ||
    (futuresSqueezeRisk && !allowPumpOverride)
      ? "AI-3_RISK"
      : null,
  ...
  const retryLaterSuggestion =
    volatilityExtreme || regimePolicy.mode === "HIGH_VOLATILITY_CHAOS"
      ...
      : volumeWeak || regimePolicy.mode === "LOW_VOLUME_DEAD_MARKET"
        ...
        : regimePolicy.mode === "LOW_VOLATILITY_CALM"
          ? "Volatilite/impulse artisi geldikten sonra tekrar degerlendir."
        : lateEntryDetected
  ...
  const alignedFactors = unique([
    ...
    futuresRiskScore > 0 && futuresRiskScore < 55 ? `Futures risk kontrollu (${futuresRiskScore.toFixed(1)})` : "",
    regimeStabilityScore >= 72 ? `Regime stabil (${regimeStabilityScore.toFixed(1)})` : "",
  ]).filter(Boolean);
  const conflictingFactors = unique([
    ...
    futuresTrapRisk ? `Futures trap/squeeze intent=${futuresIntent}` : "",
    futuresRiskScore >= 72 ? `Futures stress yuksek (${futuresRiskScore.toFixed(1)})` : "",
    unstableRegimeRisk ? `Regime unstable (${regimeLifecyclePhase})` : "",
  ]).filter(Boolean);
  let vetoBlockedBy: Array<"AI-3_RISK" | "MARKET_REGIME" | "TRADE_QUALITY" | "SELF_CRITIC"> = unique([
  ...
  const rejectReason =
    finalDecision === "NO_TRADE"
      ...
      : rr !== null && rr < minShortTermRr && !allowPumpOverride
        ? "Risk/odul orani yetersiz (min 1:2)"
  ...
  const confidenceAdjusted = Number(
    clamp(
      rawDecisionConfidence -
        ...
        regimeTransitionProbability * 0.08 -
        regimeFlipRisk * 0.06 -
        regimeChaosProbability * 0.05 +
        (regimeStabilityScore >= 72 ? 3 : 0),
      0,
      100,
    ).toFixed(2),
  );
    ...
    logger.info(
      {
        ...
        riskScore: Number(risk.score.toFixed(2)),
        riskVeto,
        qualityWeak,
        rr: rrValue,
        minShortTermRr,
        ...
        futuresTrapRisk,
        futuresIntent,
        liquiditySweepDetected: liquiditySweepRequired,
        requireLiquiditySweep,
        expansionDetected: expansionRequired,
        requireExpansion,
        entryRejectReason: rejectReason ?? liquidityRejectReason,
        noTradeReasons: noTradeReasonList,
      },
  ...
  return {
    ...
    decisionPayload: {
      ...
      futuresIntel: {
        ...
        riskScore: Number(futuresRiskScore.toFixed(2)),
        leverageStressScore: Number(leverageStressScore.toFixed(2)),
        squeezeProbability: Number(squeezeProbability.toFixed(2)),
        leveragedTrapProbability: Number(leveragedTrapProbability.toFixed(2)),
        positioningPressure: Number(input.analysisInput.marketSignals?.positioningPressure ?? 50),
        oiPriceDivergenceScore: Number(input.analysisInput.marketSignals?.oiPriceDivergenceScore ?? 0),
        manipulationPressureScore: Number(manipulationPressureScore.toFixed(2)),
        fundingRate: Number(input.analysisInput.marketSignals?.fundingRate ?? 0),
        fundingDelta: Number(input.analysisInput.marketSignals?.fundingDelta ?? 0),
        openInterest: Number(input.analysisInput.marketSignals?.openInterest ?? 0),
        openInterestDelta: Number(input.analysisInput.marketSignals?.openInterestDelta ?? 0),
        longShortRatio: Number(input.analysisInput.marketSignals?.longShortRatio ?? 0),
        liquidationImbalance: Number(input.analysisInput.marketSignals?.liquidationImbalance ?? 0),
        liquidationMagnetPrice: Number(input.analysisInput.marketSignals?.liquidationMagnetPrice ?? 0),
        liquidationMagnetDistancePercent: Number(input.analysisInput.marketSignals?.liquidationMagnetDistancePercent ?? 0),
        summary: String(input.analysisInput.marketSignals?.futuresRiskSummary ?? ""),
      },
      regimeStability: {
        stabilityScore: Number(regimeStabilityScore.toFixed(2)),
        lifecyclePhase: regimeLifecyclePhase,
        ageSec: Number(input.analysisInput.marketSignals?.regimeAgeSec ?? 0),
        transitionProbability: Number(regimeTransitionProbability.toFixed(2)),
        flipRisk: Number(regimeFlipRisk.toFixed(2)),
        chaosProbability: Number(regimeChaosProbability.toFixed(2)),
        persistenceScore: Number(input.analysisInput.marketSignals?.regimePersistenceScore ?? 50),
        switchCount10m: Number(input.analysisInput.marketSignals?.regimeSwitchCount10m ?? 0),
        chopWarning: regimeChopWarning,
        unstableBreakoutCondition: regimeUnstableBreakout,
      },
      strategyRuntime: {
      ...
      executionAction: finalDecision === "BUY" ? "OPEN" : "SKIP",
      executionReason:
        consensusDecision === "BUY"
      ...
      safeEntryPoint,
      entryRejectReason: rejectReason ?? liquidityRejectReason,
      timeframeAnalysis: toTimeframePayload(mtf) as AIDecisionPayload["timeframeAnalysis"],
    },
    generatedAt: new Date().toISOString(),
  };
}

...
export function scoreTradeOpportunity(outputs: AIProviderResult[]) {
  ...
  const weighted = valid.reduce((acc, row) => {
    ...
    return acc + (row.output!.confidence / 100) * decisionToScore(row.output!.decision) * weight;
  }, 0);
  const weightSum = valid.reduce((acc, row) => {
    const weight = Number.isFinite(row.weight) ? Math.max(0.2, Math.min(row.weight as number, 2)) : 1;
    return acc + weight;
  }, 0);
  return Number((weighted / Math.max(weightSum, 1)).toFixed(4));
}
...
export function summarizeConsensus(outputs: AIProviderResult[]): AIConsensusResult {
  ...
  const minDirectionalVotesMet = decisionCount.BUY >= 2 || decisionCount.SELL >= 2;
  ...
  const explanation = `Consensus=${finalDecision}, buy:${decisionCount.BUY}, sell:${decisionCount.SELL}, hold:${decisionCount.HOLD}, no_trade:${decisionCount.NO_TRADE}, score:${weighted}, weightedVote=${minDirectionalVotesMet ? "ok" : "insufficient"}, strict=${env.AI_STRICT_ANALYST_MODE ? "on" : "off"}, degraded=${allDegraded ? "yes" : "no"}, remoteHealthy:${remoteHealthy.length}/${healthy.length}`;
  return {
    finalDecision,
    finalConfidence: avgConfidence,
    finalRiskScore: avgRisk,

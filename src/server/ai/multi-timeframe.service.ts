...
export function buildMultiTimeframeAnalysis(input: {
  ...
  const sourceM1 = input.m1 && input.m1.length > 20 ? input.m1 : input.m5;
  const s1 = deriveTimeframeSignal("1m", sourceM1);
  ...
  const lowerOpposeCount = [s15.direction, s5.direction, s1.direction].filter((dir) => isOpposite(dir, higherTrend)).length;
  const lowerSupportCount = [s15.direction, s5.direction, s1.direction].filter((dir) => dir === higherTrend).length;
  ...
  if (higherTrend !== "RANGE" && lowerOpposeCount >= 2) {
    conflictingSignals.push(`Lower conflict 15m/5m/1m ${higherTrend} tersinde`);
  }
  ...
  const finalAlignmentSummary = conflict
    ...
    : higherUncertain
      ...
      : `MTF hizali: higher=${higherTrend}, mid=${s1h.direction}, lower=${s15.direction}/${s5.direction}/${s1.direction}, entry=${lowerEntryQuality}`;

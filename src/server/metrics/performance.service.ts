...
export async function getAdaptiveExecutionPolicy(userId?: string): Promise<AdaptiveExecutionPolicy> {
  ...
  minConfidence = Math.max(64, Math.min(68, minConfidence));
  const drawdownPressure = metrics.maxDrawdown > Math.max(5, Math.abs(metrics.netPnl) * 0.85);
  const requireUnanimous = baseUnanimous || (metrics.winRatePercent < 40 && drawdownPressure);

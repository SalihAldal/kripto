...
export class BinanceExchangeProvider implements ExchangeProvider {
  ...
  private readonly klinesCache = new Map<string, { value: KlineItem[]; at: number }>();
  private readonly orderBookCache = new Map<string, { value: OrderBookSnapshot; at: number }>();
  private readonly recentTradesCache = new Map<string, { value: RecentTrade[]; at: number }>();
  private networkFailureStreak = 0;
  private networkFailureWindowStartAt = 0;
  private readonly exchangeInfoCachePath = path.join(process.cwd(), "data", "exchange-info-tr.json");
  ...
  constructor() {
    ...
    const limiterRate = this.platform === "tr" ? 1 : 20;
    const limiterSpacingMs = this.platform === "tr" ? 350 : 0;
    this.limiter = new SimpleRateLimiter(limiterRate, 1000, limiterSpacingMs);
  ...
  private rememberKlines(key: string, value: KlineItem[]) {
    if (!value.length) return;
    this.klinesCache.set(key, { value, at: Date.now() });
  }

  private getCachedKlines(key: string, maxAgeMs: number) {
  ...
  private rememberOrderBook(key: string, value: OrderBookSnapshot) {
    if (!value.bids.length || !value.asks.length) return;
    this.orderBookCache.set(key, { value, at: Date.now() });
  }

  private getCachedOrderBook(key: string, maxAgeMs: number) {
    const cached = this.orderBookCache.get(key);
    if (!cached) return null;
    if (Date.now() - cached.at > maxAgeMs) return null;
    return {
      lastUpdateId: cached.value.lastUpdateId,
      bids: cached.value.bids.map((row) => ({ ...row })),
      asks: cached.value.asks.map((row) => ({ ...row })),
    };
  }

  private rememberRecentTrades(key: string, value: RecentTrade[]) {
    if (!value.length) return;
    this.recentTradesCache.set(key, { value, at: Date.now() });
  }

  private getCachedRecentTrades(key: string, maxAgeMs: number) {
    const cached = this.recentTradesCache.get(key);
    if (!cached) return null;
    if (Date.now() - cached.at > maxAgeMs) return null;
    return cached.value.map((row) => ({ ...row }));
  }
  ...
  private markNetworkIssueFromError(error: unknown) {
    ...
    const rateLimited =
      ...
      lower.includes("tr api code=1008");
    if (rateLimited) {
      const now = Date.now();
      const until = now + 180_000;
      if (until > this.globalNetworkIssueUntil + 5_000) {
        this.globalNetworkIssueUntil = until;
        if (this.shouldLogWarn("binance-rate-limit-cooldown", 15_000)) {
          logger.warn(
            { until, iso: new Date(until).toISOString() },
            "Binance rate-limit cooldown activated",
          );
        }
    ...
    if (this.platform === "tr" && lower.includes("www.binance.tr")) {
      return;
    }
  ...
  private async readExchangeInfoCacheFile(): Promise<ExchangeInfoResponse | null> {
    try {
      const raw = await readFile(this.exchangeInfoCachePath, "utf8");
      const parsed = JSON.parse(raw) as { savedAt?: string; data?: ExchangeInfoResponse };
      if (!parsed?.data) return null;
      if (!Array.isArray(parsed.data.symbols) || parsed.data.symbols.length === 0) return null;
  ...
  private async writeExchangeInfoCacheFile(info: ExchangeInfoResponse) {
    try {
      if (!Array.isArray(info.symbols) || info.symbols.length === 0) return;
      const dir = path.dirname(this.exchangeInfoCachePath);
      await mkdir(dir, { recursive: true });
      await writeFile(
        this.exchangeInfoCachePath,
        JSON.stringify({ savedAt: new Date().toISOString(), data: info }),
        "utf8",
      );
    } catch {
  ...
  private toGlobalSymbol(symbol: string) {
    const normalized = symbol.toUpperCase();
    if (normalized.endsWith("TRY")) return `${normalized.slice(0, -3)}USDT`;
    return normalized;
  }

  private async fetchGlobalPublicJson<T>(
    path: string,
    query?: Record<string, string | number | undefined>,
    timeoutMs?: number,
  ): Promise<T> {
    const base = (env.BINANCE_GLOBAL_HTTP_BASE ?? "https://api.binance.com").replace(/\/+$/, "");
    ...
    const effectiveTimeout = timeoutMs ?? Math.min(env.BINANCE_TIMEOUT_MS ?? 5000, 4000);
  ...
  private async fetchPublicJson<T>(
    ...
    const trPublicBases = orderedBases.filter((base) => this.isTrPublicApiV3Base(base, path));
    const nonTrBases = orderedBases.filter((base) => !this.isTrPublicApiV3Base(base, path));
    const bases =
      this.platform === "tr" && trPublicBases.length > 0 && nonTrBases.length > 0
        ? [...nonTrBases, ...trPublicBases]
        : orderedBases;
    if (bases.length !== orderedBases.length && this.shouldLogWarn("tr-public-v3-deprioritize-binance-tr", 60_000)) {
      logger.info(
        { path, deprioritizedBases: trPublicBases },
        "TR mode: deprioritizing binance.tr api/v3 base; will try after primary bases",
      );
    }
    ...
    for (const base of bases) {
      ...
      try {
        ...
        if (lower.includes("http 429") || lower.includes("tr api code=1008")) {
          this.setBaseCooldown(base, 180_000);
        }
  ...
  async getTicker(symbol: string) {
    ...
    if (this.platform === "tr") {
      if (this.isGlobalNetworkCooldownActive()) {
        const cached = this.getCachedTicker(normalized);
        if (cached) return cached;
      }
      for (const candidate of this.symbolVariants(normalized)) {
        try {
      ...
      for (const candidate of this.symbolVariants(normalized)) {
        try {
          const klines = await this.getKlines(candidate, "1h", 26);
          const usable = klines.length >= 2 ? klines : await this.getKlines(candidate, "15m", 96);
          if (usable.length < 2) continue;
          const firstOpen = usable[0]?.open ?? 0;
          const lastClose = usable[usable.length - 1]?.close ?? 0;
          if (!Number.isFinite(firstOpen) || firstOpen <= 0 || !Number.isFinite(lastClose) || lastClose <= 0) continue;
          const volume24h = usable.reduce((acc, row) => acc + Number(row.volume ?? 0) * Number(row.close ?? 0), 0);
          const change24h = ((lastClose - firstOpen) / firstOpen) * 100;
          const derived = {
            symbol: candidate,
            price: lastClose,
    ...
    if (this.isGlobalNetworkCooldownActive()) {
      ...
      const cached = this.getCachedTicker(normalized);
      return cached ?? this.fallbackTicker(normalized);
    }
    try {
  ...
  async listTickers24h() {
    try {
      const rows = await this.fetchPublicJson<Array<Record<string, string>>>(
        "/api/v3/ticker/24hr",
        undefined,
        Math.max(3500, env.BINANCE_TIMEOUT_MS ?? 5000),
      );
      const normalized = rows
        .map((row) => ({
          symbol: String(row.symbol ?? "").toUpperCase(),
          price: Number(row.lastPrice ?? row.weightedAvgPrice ?? 0),
          change24h: Number(row.priceChangePercent ?? 0),
          volume24h: Number(row.quoteVolume ?? row.volume ?? 0),
        }))
        .filter((row) => row.symbol && Number.isFinite(row.price) && row.price > 0)
        .filter((row) => (this.platform === "tr" ? row.symbol.endsWith("TRY") : true))
        .filter((row) => !row.symbol.includes("UPUSDT") && !row.symbol.includes("DOWNUSDT") && !row.symbol.includes("BULLUSDT") && !row.symbol.includes("BEARUSDT"));
      for (const row of normalized) {
        this.rememberTicker(row);
      }
      return normalized;
    } catch (error) {
      logger.warn({ error: (error as Error).message }, "24h ticker universe fetch failed");
      return [];
    }
  }
  ...
  async getKlines(symbol: string, interval = "1m", limit = 100): Promise<KlineItem[]> {
    ...
    if (this.platform === "tr") {
      ...
      const cached = this.getCachedKlines(cacheKey, 30_000);
      if (cached) return cached;
      if (this.isGlobalNetworkCooldownActive()) {
        const stale = this.getCachedKlines(cacheKey, 120_000);
      ...
      try {
        ...
        throw new Error("TR open klines empty");
      } catch (error) {
        this.markGlobalBanFromError(error);
        this.markNetworkIssueFromError(error);
        try {
  ...
  async getOrderBook(symbol: string, limit = 50): Promise<OrderBookSnapshot> {
    ...
    if (this.platform === "tr") {
      ...
      const cached = this.getCachedOrderBook(cacheKey, 8_000);
      if (cached) return cached;
      if (this.isGlobalNetworkCooldownActive()) {
        const stale = this.getCachedOrderBook(cacheKey, 60_000);
        if (stale) return stale;
      ...
      try {
        ...
        if (bids.length > 0 && asks.length > 0) {
          ...
          this.rememberOrderBook(cacheKey, parsed);
          return parsed;
        }
        throw new Error("TR open depth empty");
      } catch (error) {
        this.markGlobalBanFromError(error);
        this.markNetworkIssueFromError(error);
        try {
  ...
  async getRecentTrades(symbol: string, limit = 50): Promise<RecentTrade[]> {
    ...
    if (this.platform === "tr") {
      const cacheKey = `${normalized}|${limit}`;
      const cached = this.getCachedRecentTrades(cacheKey, 8_000);
      if (cached) return cached;
      if (this.isGlobalNetworkCooldownActive()) {
        const stale = this.getCachedRecentTrades(cacheKey, 60_000);
        if (stale) return stale;
      ...
      try {
        ...
        if (rows.length > 0) {
          ...
          this.rememberRecentTrades(cacheKey, parsed);
          return parsed;
        }
        throw new Error("TR open trades empty");
      } catch (error) {
        this.markGlobalBanFromError(error);
        this.markNetworkIssueFromError(error);
        try {
  ...
  async getExchangeInfo(): Promise<ExchangeInfoResponse> {
    const cachedExchangeInfo = this.cachedExchangeInfo;
    const hasCachedSymbols = Boolean(cachedExchangeInfo?.symbols?.length);
    const isFresh = hasCachedSymbols && Date.now() - this.cacheAt < 60_000;
    if (isFresh && cachedExchangeInfo) return cachedExchangeInfo;
    // If exchange is in temporary cooldown, use stale cache instead of failing execution flow.
    if ((this.isGlobalCooldownActive() || this.isGlobalNetworkCooldownActive()) && cachedExchangeInfo?.symbols?.length) {
    ...
    if (this.platform === "tr") {
      ...
      try {
        ...
        const nextExchangeInfo = {
          timezone: "Europe/Istanbul",
          serverTime: Date.now(),
          symbols: symbols.filter((row) => row.symbol && row.baseAsset && row.quoteAsset),
        };
        if (nextExchangeInfo.symbols.length === 0) {
          throw new Error("TR open symbols empty");
        }
        this.cachedExchangeInfo = nextExchangeInfo;
        this.cacheAt = Date.now();
        await this.writeExchangeInfoCacheFile(this.cachedExchangeInfo);
        return this.cachedExchangeInfo;
      } catch (error) {
        if (this.cachedExchangeInfo?.symbols?.length) {
          if (this.shouldLogWarn("tr-open-symbols-stale-cache", 15_000)) {
            logger.warn(
              { error: (error as Error).message, ageMs: Date.now() - this.cacheAt },
              "TR open symbols failed, using cached data",
            );
          }
          return this.cachedExchangeInfo;
        }
        const fileCache = await this.readExchangeInfoCacheFile();
        if (fileCache) {
          this.cachedExchangeInfo = fileCache;
          this.cacheAt = Date.now();
          return this.cachedExchangeInfo;
        }
        if (this.shouldLogWarn("tr-open-symbols-fallback", 15_000)) {
          logger.warn({ error: (error as Error).message }, "TR open symbols failed, falling back to exchangeInfo v3");
        }
      }
      try {
        const raw = (await this.fetchPublicJson("/api/v3/exchangeInfo")) as {
          timezone: string;
          serverTime: number;
        ...
        const nextExchangeInfo = { timezone: raw.timezone, serverTime: raw.serverTime, symbols };
        if (nextExchangeInfo.symbols.length === 0) {
          throw new Error("TR exchangeInfo empty");
        }
        this.cachedExchangeInfo = nextExchangeInfo;
        this.cacheAt = Date.now();
        await this.writeExchangeInfoCacheFile(this.cachedExchangeInfo);
        ...
        if (this.cachedExchangeInfo?.symbols?.length) {
          if (this.shouldLogWarn("tr-exchangeinfo-stale-cache", 15_000)) {
            logger.warn(
              { error: (error as Error).message, ageMs: Date.now() - this.cacheAt },
              "TR exchangeInfo failed, using cached data",
            );
          }
          return this.cachedExchangeInfo;
        }
  ...
  async estimateFees(symbol: string, side: "BUY" | "SELL", quantity: number, price: number): Promise<FeeEstimate> {
    ...
    const takerFeeRate = resolveBinanceTakerFeeRate();
    const makerFeeRate = resolveBinanceMakerFeeRate();
    return {
      symbol: symbol.toUpperCase(),
      side,
      quantity,

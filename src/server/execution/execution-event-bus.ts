...
function resolveActionType(event: ExecutionStatusEvent): ActionType | null {
  ...
  if (stage === "round-engine") return "round_status";
  if (stage === "position-report") return "position_report";
...
export function publishExecutionEvent(event: Omit<ExecutionStatusEvent, "createdAt">) {
  ...
  const shouldPersist =
    ...
    payload.level === "ERROR" ||
    payload.level === "WARN" ||
    payload.stage === "round-engine" ||
    payload.stage === "position-report" ||
    actionType !== null;
  if (shouldPersist) {
    void writeStructuredLog({
      ...
      actionType: actionType ?? "round_status",

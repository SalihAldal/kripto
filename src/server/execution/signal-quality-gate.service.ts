...
export function evaluateSignalQualityGate(input: {
  ...
  const pumpStage = String(candidate.context.metadata.pumpBreakoutStage ?? "");
  const pumpPriorityScore = Number(
    candidate.context.metadata.topGainerPriorityScore ?? candidate.context.metadata.pumpEarlyPriorityScore ?? 0,
  );
  const pumpCandidate = Boolean(
    candidate.context.metadata.pumpEarlyConfirmed ||
      candidate.context.metadata.pumpContinuationMode ||
      candidate.context.metadata.pumpIntradaySpike ||
      pumpPriorityScore >= 70,
  );
  ...
  const regimeStabilityScore = Number(candidate.context.metadata.regimeStabilityScore ?? ai.decisionPayload?.regimeStability?.stabilityScore ?? 50);
  const regimeTransitionProbability = Number(candidate.context.metadata.regimeTransitionProbability ?? ai.decisionPayload?.regimeStability?.transitionProbability ?? 0);
  const regimeFlipRisk = Number(candidate.context.metadata.regimeFlipRisk ?? ai.decisionPayload?.regimeStability?.flipRisk ?? 0);
  const regimeChaosProbability = Number(candidate.context.metadata.regimeChaosProbability ?? ai.decisionPayload?.regimeStability?.chaosProbability ?? 0);
  const regimeLifecyclePhase = String(candidate.context.metadata.regimeLifecyclePhase ?? ai.decisionPayload?.regimeStability?.lifecyclePhase ?? "UNKNOWN");
  const regimeChopWarning = Boolean(candidate.context.metadata.regimeChopWarning ?? ai.decisionPayload?.regimeStability?.chopWarning ?? false);
  const regimeUnstableBreakout = Boolean(candidate.context.metadata.regimeUnstableBreakoutCondition ?? ai.decisionPayload?.regimeStability?.unstableBreakoutCondition ?? false);
  const futuresRiskScore = Number(candidate.context.metadata.futuresRiskScore ?? ai.decisionPayload?.futuresIntel?.riskScore ?? 0);
  const leverageStressScore = Number(candidate.context.metadata.leverageStressScore ?? ai.decisionPayload?.futuresIntel?.leverageStressScore ?? 0);
  const leveragedTrapProbability = Number(candidate.context.metadata.leveragedTrapProbability ?? ai.decisionPayload?.futuresIntel?.leveragedTrapProbability ?? 0);
  const futuresIntent = String(candidate.context.metadata.futuresIntent ?? ai.decisionPayload?.futuresIntel?.intent ?? "NEUTRAL");
  const liquidityRejectReason = String(ai.decisionPayload?.entryRejectReason ?? "");
  const safeEntryTiming = String(ai.decisionPayload?.liquidityIntel?.safeEntryTiming ?? "");
  const sweepDetected = Boolean(ai.decisionPayload?.liquidityIntel?.sweepDetected ?? false);
  ...
  const pumpMomentumStrong =
    candidate.context.change24h >= 12 &&
    shortMomentum >= 0.18 &&
    shortFlow >= 0.1;
  const pumpQualityCandidate =
    pumpCandidate &&
    (pumpStage === "EARLY" || pumpStage === "ACTIVE" || pumpPriorityScore >= 72) &&
    pumpMomentumStrong;
  ...
  const adaptiveReduction =
    ...
    (marketRegime === "RANGE_SIDEWAYS" ||
      ...
      ? 8
      : 0;
  const minRequiredScore = clamp(
    baseMinRequiredScore - adaptiveReduction - (pumpQualityCandidate ? 4 : 0),
    48,
    baseMinRequiredScore,
  );
  ...
  const minShortTermRr = Math.max(env.AI_HYBRID_MIN_RR_RATIO, 1.1);
  const rrScore = rr <= 0 ? 0 : clamp((rr / Math.max(minShortTermRr, 0.01)) * 100, 0, 100);
  ...
  const liquidityState = clamp(
    liquidityBase -
      ...
      futuresRiskScore * 0.16 -
      leverageStressScore * 0.1,
    0,
    100,
  );
  ...
  const marketRegimeAlignment = clamp(
    (marketRegime === "STRONG_BULLISH_TREND" || marketRegime === "WEAK_BULLISH_TREND" ? 80 : 0) +
      ...
      (marketRegime === "LOW_VOLATILITY_CALM" ? 56 : 0) +
      ...
      (regimeStabilityScore >= 72 ? 8 : 0) -
      regimeTransitionProbability * 0.18 -
      regimeFlipRisk * 0.14 -
      regimeChaosProbability * 0.12,
    0,
    100,
  );
  ...
  const riskRewardQuality = clamp(riskReward - leveragedTrapProbability * 0.12, 0, 100);
  ...
  if (marketRegime === "LOW_VOLATILITY_CALM") {
    reasons.push("Low volatility modu: breakout kovalamak uygun degil");
    if (!pumpQualityCandidate && Math.abs(shortMomentum) < 0.08) {
      hardRejectReasons.push("Low volatility + momentum zayif");
    }
  }
  ...
  if (safeEntryTiming.includes("WAIT") || safeEntryTiming.includes("NO_ENTRY")) {
    reasons.push("Confirmation bekleniyor (safe entry timing)");
    hardRejectReasons.push("Confirmation bekleniyor");
  }
  if (sweepDetected && safeEntryTiming.includes("STANDARD")) {
    reasons.push("Sweep sonrasi teyit yok");
    hardRejectReasons.push("Sweep teyidi yok");
  }
  ...
  if (futuresRiskScore >= 78 || leveragedTrapProbability >= 82) {
    reasons.push(`Futures leverage riski yuksek (${futuresIntent}, risk=${futuresRiskScore.toFixed(1)})`);
    hardRejectReasons.push("Futures leverage/trap risk");
  } else if (futuresRiskScore >= 60 || leverageStressScore >= 68) {
    reasons.push(`Futures stress caution (${futuresIntent}, stress=${leverageStressScore.toFixed(1)})`);
  }
  if (regimeTransitionProbability >= 62 || regimeFlipRisk >= 68 || regimeChopWarning) {
    reasons.push(`Regime stability caution (${regimeLifecyclePhase}, transition=${regimeTransitionProbability.toFixed(1)})`);
  }
  if (regimeChopWarning || regimeUnstableBreakout || regimeChaosProbability >= 82) {
    hardRejectReasons.push("Unstable/chop regime");
  }

...
function formatOptionalNumber(value: unknown, digits: number) {
  const num = toOptionalNumber(value);
  return num === null ? "-" : num.toFixed(digits);
}

function resolveEventQuantity(event: TradeLifecycleEvent) {
  return event.context?.quantity ?? event.context?.requestedSellQty ?? event.context?.filledQty ?? event.context?.buyQuantity;
}

function resolveEventPrice(event: TradeLifecycleEvent) {
  return (
    event.context?.targetSellPrice ??
    event.context?.executionPrice ??
    event.context?.exitPrice ??
    event.context?.buyPrice ??
    event.context?.buyEntryPrice ??
    event.context?.entryPrice
  );
}
...
function toSellStatusLabel(event: TradeLifecycleEvent) {
  if (event.stage === "scanner-start" || event.stage === "scanner") return "Taraniyor";
          ...
          <div className="w-full max-w-3xl rounded-xl border border-outline-variant/30 bg-surface p-5">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              <div className="rounded-md border border-outline-variant/20 bg-surface-container-low px-3 py-2">
                ...
                <p className="font-semibold">{formatOptionalNumber(resolveEventQuantity(selected), 8)}</p>
              </div>
              <div className="rounded-md border border-outline-variant/20 bg-surface-container-low px-3 py-2">
                <p className="text-on-surface-variant">Fiyat</p>
                <p className="font-semibold">
                  {formatOptionalNumber(resolveEventPrice(selected), 6)}
                  {toOptionalNumber(resolveEventPrice(selected)) === null ? "" : " TL"}
                </p>
              </div>
              <div className="rounded-md border border-outline-variant/20 bg-surface-container-low px-3 py-2">

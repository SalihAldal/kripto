...
export function sourceFingerprint() {
    const files = execFileSync("git", ["ls-files", "-z", "--cached", "--others", "--exclude-standard", "--", "src/server", "services", "lib", "app/api", "prisma", "scripts", "package.json", "package-lock.json", "tsconfig.json", "tsconfig.strategy-tools.json"], { encoding: "utf8" }).split("\0").filter(Boolean);

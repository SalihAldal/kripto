/**
 * 10-hour Alpha V2 PAPER forward validation — gated launcher.
 * Refuses to start unless all paper-readiness gates pass.
 * Usage: npx tsx scripts/run-10h-paper-alpha-v2.ts
 */
...
const DURATION_MS = 600 * 60 * 1000;
const CAMPAIGN_ID = `paper-alpha-v2-10h-${new Date().toISOString().replace(/[:.]/g, "-")}`;
const RESULT_FILE = "kripto-alpha-v2-10h-paper-result.json";
...
function assertPaperGates() {
  if (!fs.existsSync(SCOREBOARD)) {
    throw new Error("PAPER_GATE_BLOCKED: alpha-scoreboard.json missing — run alpha experiments first");
  }
  ...
  if (!ready) {
    throw new Error(
      "PAPER_GATE_BLOCKED: no alpha passed validation + final + robustness with positive realistic-cost edge",
    );
  }
  return ready;
}
...
async function main() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ alphaId: alpha.alphaId, frozenAt: new Date().toISOString() }, null, 2),
  );

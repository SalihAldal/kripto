...
import { listTradeJournal } from "@/src/server/services/trade-journal.service";
...
export async function openTrade(input: {
  ...
  return executeAnalyzeAndTrade({
    ...
    preselectedCandidate: input.preselectedCandidate,
...
export async function listDetailedTradeLogs(input?: {
  limit?: number;
  positionId?: string;
  symbol?: string;
  eventType?: string;
}) {
  return listTradeEventLogs(input);
}

export async function listJournal(input: { userId?: string; limit?: number; mode?: string | null }) {
  const { user } = await getRuntimeExecutionContext(input.userId);
  return listTradeJournal({ userId: user.id, limit: input.limit, mode: input.mode });
}
...
export async function getTradeRoundHistory(
  userId?: string,
  input?: {
    page?: number;
    pageSize?: number;
  ...
  return getAutoRoundRunsHistory(userId, input ?? {});
}
...
export async function deleteTradeRoundRun(runId: string, userId?: string) {
  if (!runId || runId.trim().length < 6) {
    return { deleted: false, reason: "Gecersiz run id." };
  }
  if (!userId) {
    return { deleted: false, reason: "Kullanici bulunamadi." };
  }
  return removeAutoRoundRun(userId, runId);
}

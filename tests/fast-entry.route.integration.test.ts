...
describe("fast-entry route integration", () => {
  it("analiz -> coin secim -> alim akisinda basarili doner", async () => {
    ...
    const req = new NextRequest("http://localhost/api/trades/fast-entry", {
      method: "POST",
      body: JSON.stringify({ execute: true, amountTry: 1000, scanLimit: 100 }),
    ...
    expect(openTradeMock).toHaveBeenCalledWith(
      expect.objectContaining({
        symbol: "BTCTRY",
        amountTry: 1000,
        preselectedCandidate: expect.objectContaining({
          context: expect.objectContaining({ symbol: "BTCTRY" }),
        }),
      }),
    );
